...
let appointmentsArr = localStorage.getItem('Appointments')
...
// Toggles a green border around calendar cells when clicked and renders the selected appointment
...
document.querySelector('#CalendarPage').addEventListener('click', e => {
  ...
  const appointmentDaily = appointments.filter(
    appointment => appointment.date === e.target.attributes.date.textContent
  )
...
function editAppointment (day) {
  ...
  const editedArr = appointmentsArr.filter(
    appointment => appointment.date !== appointmentObj.date
  )
...
function onSubmit (e) {
  ...
  const daySelect =
    formInputs.elements[0].valueAsDate.getMonth() === 10
      ? '1'
      : (formInputs.elements[0].valueAsDate.getDate() + 1).toString()
  ...
  for (appointment of appointmentsArr) {
    if (appointmentObj.date === appointment.date) {
      alert(
        'You have already scheduled an appointment for this day. You can delete to add a different appointment or edit the current one.'
      )
    }
  }
